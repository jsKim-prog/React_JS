...
const ModifyComponent = () => {
    ...
    const liginInfo = useSelector(state => state.loginSlice)
    const [result, setResult] = useState()
    const {moveToLogin} = useCustomLogin()

    useEffect(() => {
        setMember({ ...liginInfo, pw: 'ABCD' })
    }, [liginInfo])
    ...
    return (
        <div className="mt-6">
            {result? <ResultModal title={'회원정보'} content={'정보수정 완료'} callbackFn={closeModal}></ResultModal>:<></>}
