...
const ModifyComponent = ({ pno }) => {
    ...
    const {moveToList, moveToRead} = useCustomMove() //이동용
    ...
    const deleteOldImages = (imageName)=>{
        const resulFileNames = product.uploadFileNames.filter(fileName=> fileName !==imageName)
        product.uploadFileNames = resulFileNames // 현재 이미지가 아닌 이미지 파일명만 세팅
        setProduct({...product})
    }
